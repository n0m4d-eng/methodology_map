...
- `pspy64` (transfer from attacker) to monitor hidden cron jobs

- Write access to at least one script or directory in the cron execution path

- One of the most common Linux privesc paths in exam environments — a root-owned script in a world-writable location, or a cron script calling a binary by bare name without a full path

- `pspy` monitors process creation without root and catches jobs that don't appear in any crontab file (systemd timers, user crontabs, etc.)
...
- Cron script modified → SUID bash planted → `/bin/bash -p` → `root-linux`

- Wildcard injection fires → SUID or reverse shell → root

- PATH hijack succeeds → custom binary runs as root → `root-linux`

- Watch pspy for 1-2 minutes — many jobs run every minute
